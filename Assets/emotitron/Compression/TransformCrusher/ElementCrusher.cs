...
namespace emotitron.Compression
{
	...
	public class ElementCrusher
	{
		...
		public Bounds Bounds
		{
			get
			{
				bounds.SetMinMax(new Vector3(xcrusher.Min, ycrusher.Min, zcrusher.Min), new Vector3(xcrusher.Max, ycrusher.Max, zcrusher.Max));
				return bounds;
			}
		...
		/// Applies only the enabled axes to the transform, leaving the disabled axes untouched.
		/// </summary>
		public void Apply(Rigidbody rb, Element e)
		{
			if (!cached)
				CacheValues();
			...
			switch (_trsType)
			{
				...
				case TRSType.Position:
					...
					rb.position = new Vector3(
						cache_xEnabled ? e.v.x : rb.position.x,
						cache_yEnabled ? e.v.y : rb.position.y,
						cache_zEnabled ? e.v.z : rb.position.z
						);
					return;
				...
				case TRSType.Euler:
						...
						rb.rotation = Quaternion.Euler(
							cache_xEnabled ? e.v.x : rb.rotation.eulerAngles.x,
							cache_yEnabled ? e.v.y : rb.rotation.eulerAngles.y,
							cache_zEnabled ? e.v.z : rb.rotation.eulerAngles.z
							);
					return;
	...
	public class ElementCrusherDrawer : CrusherDrawer
	{
		...
		public override void OnGUI(Rect r, SerializedProperty property, GUIContent label)
		{
			...
			{
				GUIContent title = target.TRSType == TRSType.Generic ? new GUIContent(property.displayName) : new GUIContent(System.Enum.GetName(typeof(TRSType), target.TRSType)); // + " Crshr");
